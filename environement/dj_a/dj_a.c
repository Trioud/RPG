...
{
    if (env->perso_pos.x >= 378 && env->perso_pos.x <= 458 &&
        env->perso_pos.y >= 270 && env->perso_pos.y <= 350){
        env->dj = 1;
        if (start_cbt("mine_boss", env) == 2){
            env->nb_dj_fini += 1;
            end_cbt(2);
        }
        else
...
void draw_dj_a(sfRenderWindow *win, env_t *env)
{
    sfRenderWindow_clear(win, sfBlack);
    sfRenderWindow_drawSprite(win, env->dj_a, NULL);
    int_dja(win, env);
...
void event_dj_a(sfRenderWindow *win, env_t *env)
{
